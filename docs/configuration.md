...
FM_CREW_STATE_CONTEXT=1       # set to 0 to suppress optional context telemetry in fm-crew-state.sh output
...
FM_ROTATE_THRESHOLD=70        # context-fullness percentage that makes a non-busy turn boundary queue rotation-due
FM_CONTEXT_CAPTURE_LINES=12   # pane tail lines scanned for conservative context telemetry
FM_ROTATE_WAIT_SECS=0         # seconds fm-rotate waits for a committed handoff and clean idle boundary; 0 requests only
FM_ROTATE_WAIT_POLL_SECS=10   # seconds between fm-rotate handoff/idle checks
FM_ROTATE_SEND_RETRIES=3      # submit verification attempts used by fm-rotate when sending prompts/exit commands
FM_ROTATE_SEND_SLEEP=0.4      # seconds between fm-rotate submit verification attempts
FM_ROTATE_EXIT_ACK_TIMEOUT=3  # seconds fm-rotate waits for verified shell readiness when exit-command submit confirmation stays pending/unknown
FM_ROTATE_EXIT_ACK_POLL_SECS=1  # seconds between exit-ack shell-readiness checks
FM_ROTATE_EXIT_TIMEOUT=30     # seconds fm-rotate waits for a verified shell after the old harness exit command before relaunch
FM_ROTATE_EXIT_POLL_SECS=1    # seconds between shell-readiness checks during rotation
FM_ROTATE_LAUNCH_TIMEOUT=20   # seconds fm-rotate waits for the fresh harness launch to leave the verified shell
FM_ROTATE_LAUNCH_POLL_SECS=1  # seconds between fresh-harness launch checks during rotation
FM_ROTATE_BUSY_CAPTURE_LINES=80  # pane tail lines checked for a busy signature immediately before exit
FM_RUN_HARNESS=               # optional default harness for bin/fm-run.sh when no --harness/command is supplied
FM_RUN_COMMAND=               # optional shell command string for bin/fm-run.sh instead of a harness default
FM_RUN_START_PROMPT=          # optional startup prompt for fm-run's default harness launch; default runs fm-session-start and resumes supervision
FM_RUN_STOP_FILE=state/.fm-run-stop  # sentinel path that tells fm-run not to relaunch after harness exit
FM_RUN_MIN_RUNTIME_SECS=30    # session lifetime below which fm-run treats an exit as a crash loop
FM_RUN_BACKOFF_SECS=10        # initial fm-run relaunch backoff after a too-fast exit
FM_RUN_BACKOFF_MAX_SECS=300   # maximum fm-run relaunch backoff
...
FM_BUSY_REGEX='esc (to )?interrupt|Working\.\.\.|Ctrl\+c:cancel|(…|\.\.\.)[[:space:]]*\([0-9]+s[[:space:]]*[^)]*(thinking|tokens?)[^)]*\)'   # busy-pane signatures, shared by watcher, daemon, fm-crew-state pane fallback, and tmux helper
