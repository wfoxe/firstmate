...
| Send key | `zellij action send-keys --pane-id <id> <key>` | Verified names: `"Enter"` (also `"enter"`) works; `"Esc"`/`"esc"` work but `"Escape"`/`"escape"` are REJECTED with "Invalid key"; Ctrl-C and Ctrl-Q must be single shell arguments (`"Ctrl c"` and `"Ctrl q"`, respectively) - `"C-c"`, `"Ctrl+c"`, and passing `Ctrl`/`c` as two separate argv words all fail. Resolves report gap #2. |
...
- **Soft context rotation cannot relaunch zellij tasks yet.** `fm-rotate.sh` must prove the old harness has exited back to a shell in the task worktree before sending the continuation launch. Zellij has no passive live-cwd or shell-readiness primitive; the spawn-time `pwd` probe is deliberately not reused after an agent has been running because injecting probe commands into an unknown post-exit pane could corrupt the next launch.
